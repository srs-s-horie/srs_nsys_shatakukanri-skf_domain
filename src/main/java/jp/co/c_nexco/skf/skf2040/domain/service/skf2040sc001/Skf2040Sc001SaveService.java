...
public class Skf2040Sc001SaveService extends SkfServiceAbstract<Skf2040Sc001SaveDto> {
	...
	private boolean execSave(Skf2040Sc001SaveDto saveDto) {
		...
		if (CodeConstant.STATUS_MISAKUSEI.equals(saveDto.getApplStatus())) {
			...
			Skf2040TTaikyoReport taikyoInfo = skf2040Sc001SharedService.getExistTaikyoInfo(saveDto.getApplNo());
			if (taikyoInfo == null) {
				...
				LogUtils.infoByMsg("一時保存ボタン押下時： 退居届情報を取得失敗   申請書番号：" + saveDto.getApplNo());
				return false;
			}
