...
public class Skf2040Sc001InitService extends BaseServiceAbstract<Skf2040Sc001InitDto> {
	...
	public void setLoginUserInfo(Skf2040Sc001InitDto initDto) {
		...
		Map<String, String> loginUserInfoMap = skfLoginUserInfoUtils.getAlterLoginUserInfoMap(menuScopeSessionBean);
		...
		// ユーザに関する追加情報を取得
		Skf2040Sc001GetShainInfoExp shainInfo = skf2040Sc001SharedService.getShainInfo(CodeConstant.C001,
				initDto.getShainNo());
		if (shainInfo == null) {
	...
	private void setDropdownList(Skf2040Sc001InitDto initDto) {
		...
		initDto.setDdlTaikyoRiyuKbnList(skf2040Sc001SharedService.getTaikyoHenkanRiyuList(initDto.getTaikyoRiyuKbn()));
	...
	protected void setInputControl(Skf2040Sc001InitDto initDto) {
		...
		if (!skfShinseiUtils.checkSKSTeijiStatus(initDto.getShainNo(), FunctionIdConstant.R0103, initDto.getApplNo())) {
			// 申請不可の場合
			// 確認ボタン非活性
