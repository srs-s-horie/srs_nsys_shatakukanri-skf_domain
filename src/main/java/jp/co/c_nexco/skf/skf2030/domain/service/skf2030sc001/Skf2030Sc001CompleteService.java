...
import jp.co.c_nexco.skf.skf2030.domain.dto.skf2030sc001.Skf2030Sc001CompleteDto;
 ...
 */
@Service
public class Skf2030Sc001CompleteService extends BaseServiceAbstract<Skf2030Sc001CompleteDto> {
	 ...
	 */
	@Override
	public Skf2030Sc001CompleteDto index(Skf2030Sc001CompleteDto dto) throws Exception {
		// 操作ログ出力
		skfOperationLogUtils.setAccessLog("搬入完了日相関チェックOK", CodeConstant.C001, FunctionIdConstant.SKF2030_SC001);

		// タイトル設定
		dto.setPageTitleKey(MessageIdConstant.SKF2030_SC001_TITLE);

		// 申請情報設定
		Map<String, String> applInfo = new HashMap<String, String>();
		applInfo.put("status", dto.getSendApplStatus());
	 ...
	 * @throws Exception
	 */
	private boolean checkDispInfo(Skf2030Sc001CompleteDto dto) throws Exception {
		boolean result = true;
		// 備品希望の「申請する」が選択されている場合は必須チェック必要
		boolean bihinCheckFlag = false;
		if (NfwStringUtils.isEmpty(dto.getBihinCheckFlag())) {
			bihinCheckFlag = Boolean.parseBoolean(dto.getBihinCheckFlag());
		}
		...
		// 必須入力確認
		result = checkControlEmpty(dto, validateFlag, errorTarget);
		// バイト数確認
		result = checkByteCount(dto, validateFlag, errorTarget, result);
		// 日付確認
		result = checkDate(dto, validateFlag, errorTarget, result);

		return result;
	}

	private boolean checkDate(Skf2030Sc001CompleteDto dto, List<String> validateFlag, List<String> errorTarget,
			boolean result) {
		if (!result) {
			return result;
		}
		// 搬入希望日
		if (!dto.isBihinReadOnly() && validateFlag.indexOf("sessionDay") >= 0) {
			if (!SkfCheckUtils.isSkfDateFormat(dto.getSessionDay().trim(), CheckUtils.DateFormatType.YYYYMMDD)) {
				errorTarget.add("sessionDay");
				ServiceHelper.addErrorResultMessage(dto, new String[] { "comment" }, MessageIdConstant.E_SKF_1055,
						"搬入希望日");
				// ServiceHelper.addErrorResultMessage(dto,
		...
		// 搬入完了日
		if (dto.isBihinReadOnly()) {
			if (!SkfCheckUtils.isSkfDateFormat(dto.getCompletionDay(), CheckUtils.DateFormatType.YYYYMMDD)) {
				errorTarget.add("completionDay");
				ServiceHelper.addErrorResultMessage(dto, new String[] { "completionDay" }, MessageIdConstant.E_SKF_1055,
						"搬入完了日");
				// ServiceHelper.addErrorResultMessage(dto,
				// errorTarget.toArray(new String[errorTarget.size()]),
	...
	private boolean checkByteCount(Skf2030Sc001CompleteDto dto, List<String> validateFlag, List<String> errorTarget,
			boolean result) throws Exception {
		if (dto.isStatus01Flag() && result) {
			int commentMaxCnt = Integer.parseInt(PropertyUtils.getValue("skf2030.skf2030_sc001.comment_max_count"));
			if (CheckUtils.isMoreThanByteSize(dto.getCommentNote(), commentMaxCnt)) {
				errorTarget.add("comment");
				ServiceHelper.addErrorResultMessage(dto, new String[] { "comment" }, MessageIdConstant.E_SKF_1049,
	...
	private boolean checkControlEmpty(Skf2030Sc001CompleteDto dto, List<String> validateFlag,
			List<String> errorTarget) {
		boolean result = true;
		// コメント
		if (!dto.isBihinReadOnly() && validateFlag.indexOf("tel") >= 0) {
			if (NfwStringUtils.isEmpty(dto.getTel())) {
				errorTarget.add("tel");
				ServiceHelper.addErrorResultMessage(dto, new String[] { "tel" }, MessageIdConstant.E_SKF_1048,
						"勤務先のTEL");
				// ServiceHelper.addErrorResultMessage(dto,
				// errorTarget.toArray(new String[errorTarget.size()]),
				// MessageIdConstant.E_SKF_1048, "勤務先のTEL");
		...
		// 連絡先
		if (!dto.isBihinReadOnly() && validateFlag.indexOf("renrakuSaki") >= 0) {
			if (NfwStringUtils.isEmpty(dto.getRenrakuSaki())) {
				errorTarget.add("renrakuSaki");
				ServiceHelper.addErrorResultMessage(dto, new String[] { "renrakuSaki" }, MessageIdConstant.E_SKF_1048,
						"連絡先");
				// ServiceHelper.addErrorResultMessage(dto,
				// errorTarget.toArray(new String[errorTarget.size()]),
		...
		// 搬入希望日
		if (!dto.isBihinReadOnly() && validateFlag.indexOf("sessionTime") >= 0) {
			if (NfwStringUtils.isEmpty(dto.getSessionTime())) {
				errorTarget.add("sessionTime");
				ServiceHelper.addErrorResultMessage(dto, new String[] { "sessionTime" }, MessageIdConstant.E_SKF_1048,
						"搬入希望時刻");
				// ServiceHelper.addErrorResultMessage(dto,
				// errorTarget.toArray(new String[errorTarget.size()]),
