...
public class Skf3090Sc001ImportService extends BaseServiceAbstract<Skf3090Sc001ImportDto> {
	...
	private enum IMPORT_COL {
		IMPORT_COL_NO("No"),
		IMPORT_COL_PREF_NAME("都道府県"),
		IMPORT_COL_MONEY("現物支給価額"),
		IMPORT_COL_EFFECTIVE_DATE("改定日");
	...
	private boolean checkValidateInput(Skf3090Sc001ImportDto importDto) {
		...
		if (fileLen > TORIKOMI_FILE_LENGTH) {
			ServiceHelper.addErrorResultMessage(importDto, null, MessageIdConstant.E_SKF_1049, "取込ファイル", TORIKOMI_FILE_LENGTH);
	...
	private boolean importGenbutsuShikyuKagaku(Skf3090Sc001ImportDto importDto) {
		...
		*/
		...
		for (int i = 0; i < (rowDataBeanList.size() - 1); i++) {
			...
			if(cellDataBeanList.size() != ROW_COUNT){
				// 項目数エラー
				ServiceHelper.addErrorResultMessage(importDto, null, MessageIdConstant.E_SKF_1095, "項目数" , cellDataBeanList.size());
				// 取り込み不可カウンタをインクリメント
				impNgRecordCount += 1;
				// 次の行へ
				continue;
			...
			if(false == isFormatCheckDetail(posMap, cellDataBeanList, importDto, rowDataList, i + 2, systemShoriNengetsu)){
				// フォーマットエラー
				// 取り込み不可カウンタをインクリメント
				impNgRecordCount += 1;
				// 次の行へ
			...
			if(existGenbutsuShikyuKagakuKeyList.size() > 0 && existGenbutsuShikyuKagakuKeyList.contains(genbutsuShikyuKagakuKey.toString())){
				// 重複エラー
				// error.skf.e_skf_1123=取込ファイル内に、キー重複を検出しました。
				ServiceHelper.addErrorResultMessage(importDto, null, MessageIdConstant.E_SKF_1123);
				// 取り込み不可カウンタをインクリメント
				impNgRecordCount += 1;
				// 次の行へ
				continue;				
		...
		if(0 == impNgRecordCount){
			...
			// infomation.skf.i_skf_1029=取込可件数（登録可）：{0}
			ServiceHelper.addWarnResultMessage(importDto, MessageIdConstant.I_SKF_1029, String.valueOf(impOkRecordCount));
			// infomation.skf.i_skf_1016=取込不可件数（登録不可）：{0}
			ServiceHelper.addWarnResultMessage(importDto, MessageIdConstant.I_SKF_1016, String.valueOf(impNgRecordCount));
		}
	 ...
	 * @return チェック結果
	 */
	private boolean isFormatCheckDetail(Map<String, Integer> posMap, List<CellDataBean> cellDataBeanList, Skf3090Sc001ImportDto importDto, List <String> rowDataList, int rowNo, String systemShoriNengetsu) {
		boolean returnFlg = true;
        ...
        // 必須チェック
        // - 適用年月日
		if((true == cellDataBeanList.get(effectiveDatePos).getValue().isEmpty()) || cellDataBeanList.get(effectiveDatePos).getValue().trim().length() == 0 ){
			// error.skf.e_skf_1096={0}列目の{0}は必須項目です。
			//ServiceHelper.addErrorResultMessage(importDto, null, MessageIdConstant.E_SKF_1096, String.valueOf(rowNo), TEKIYOU_DATE);
			ServiceHelper.addWarnResultMessage(importDto, MessageIdConstant.E_SKF_1096, String.valueOf(rowNo), TEKIYOU_DATE);
			// 戻り値として異常を設定
			returnFlg = false;
		}
		// - 都道府県コード
		if((true == cellDataBeanList.get(prefCdPos).getValue().isEmpty()) || cellDataBeanList.get(prefCdPos).getValue().trim().length() == 0 ){
			// error.skf.e_skf_1096={0}列目の{0}は必須項目です。
			//ServiceHelper.addErrorResultMessage(importDto, null, MessageIdConstant.E_SKF_1096, String.valueOf(rowNo), PREF_CD);
			ServiceHelper.addWarnResultMessage(importDto, MessageIdConstant.E_SKF_1096, String.valueOf(rowNo), PREF_CD);
			// 戻り値として異常を設定
			returnFlg = false;
		}
		...
		try {
			if(CheckUtils.isMoreThanByteSize(money.trim(),4)){
				...
				//ServiceHelper.addErrorResultMessage(importDto, null, MessageIdConstant.E_SKF_1097, String.valueOf(rowNo), JYUKYO_RIEKI_GAKU, String.valueOf(4));
				ServiceHelper.addWarnResultMessage(importDto, MessageIdConstant.E_SKF_1097, String.valueOf(rowNo), JYUKYO_RIEKI_GAKU, String.valueOf(4));
				// 戻り値として異常を設定
				returnFlg = false;
			}
		...
		if(false == money.matches("^[0-9]*$")){
			...
			//ServiceHelper.addErrorResultMessage(importDto, null, MessageIdConstant.E_SKF_1126, String.valueOf(rowNo), JYUKYO_RIEKI_GAKU);
			ServiceHelper.addWarnResultMessage(importDto, MessageIdConstant.E_SKF_1126, String.valueOf(rowNo), JYUKYO_RIEKI_GAKU);
			// 戻り値として異常を設定
			returnFlg = false;
		}
		// - 都道府県コード
		...
		if(false == prefCd.matches("^[0-9]*$")){
			// error.skf.e_skf_1126={0}列目の{1}に不正な値が設定されています。。
			//ServiceHelper.addErrorResultMessage(importDto, null, MessageIdConstant.E_SKF_1126, String.valueOf(rowNo), PREF_CD);
			ServiceHelper.addWarnResultMessage(importDto, MessageIdConstant.E_SKF_1126, String.valueOf(rowNo), PREF_CD);
			// 戻り値として異常を設定
			returnFlg = false;
		}else{
			// 半角数字の範囲チェック
		...
		if(false == effectiveDate.matches("^[0-9.]*$")){
			// 数値以外の場合は全てエラー
			// error.skf.e_skf_1126={0}列目の{1}に不正な値が設定されています。。
			//ServiceHelper.addErrorResultMessage(importDto, null, MessageIdConstant.E_SKF_1126, String.valueOf(rowNo), TEKIYOU_DATE);
			ServiceHelper.addWarnResultMessage(importDto, MessageIdConstant.E_SKF_1126, String.valueOf(rowNo), TEKIYOU_DATE);
			// 戻り値として異常を設定
			returnFlg = false;				
		}else{
			// 数値の場合のみ、年月日に変換
		...
		if(true == returnFlg){
			if((false == cellDataBeanList.get(prefNamePos).getValue().isEmpty()) || cellDataBeanList.get(prefNamePos).getValue().trim().length() > 0 ){
