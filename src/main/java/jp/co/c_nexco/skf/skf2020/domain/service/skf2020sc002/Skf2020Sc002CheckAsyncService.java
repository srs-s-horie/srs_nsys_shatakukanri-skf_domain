...
public class Skf2020Sc002CheckAsyncService extends AsyncBaseServiceAbstract<Skf2020Sc002CheckAsyncDto> {
	...
	public AsyncBaseDto index(Skf2020Sc002CheckAsyncDto checkDto) throws Exception {
		...
		if (!ret) {
			...
			applMap.put("TelErr", checkDto.getTelErr());
			resultApplList.add(applMap);
			menuScopeSessionBean.put(CommonConstant.BUSINESS_RESULT_MESSAGES, resultApplList);

			checkDto.setUpdateErrorFlg(SkfCommonConstant.PRESEMCE_NEED);
			// throwBusinessExceptionIfErrors(checkDto.getResultMessages());
			return checkDto;
		}

		checkDto.setUpdateErrorFlg(SkfCommonConstant.PRESEMCE_NOT_NEED);
		return checkDto;
	}
	...
	private boolean checkDispInfo(Skf2020Sc002CheckAsyncDto checkDto) throws Exception {
		...
		// デフォルトの状態を設定
		// skf2020Sc002SharedService.setCheckRadio(checkDto);
		// skf2020Sc002SharedService.setControlDdl(checkDto);
		// skf2020Sc002SharedService.setControlValue(checkDto);
	...
	private Boolean checkControlEmpty(Skf2020Sc002CheckAsyncDto checkDto) {
		...
		if (NfwStringUtils.isBlank(checkDto.getTel())) {
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "勤務先のTEL");
			checkDto.setTelErr(validationErrorCode);
			result = false;
		}
		...
		if (NfwStringUtils.isBlank(checkDto.getTaiyoHituyo())) {
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1054, "社宅を必要としますか？");
			checkDto.setTaiyoHituyoErr(validationErrorCode);
			result = false;
		}
		...
		if (NfwStringUtils.isNotBlank(checkDto.getTaiyoHituyo())) {
			if (CodeConstant.ASKED_SHATAKU_HITSUYO.equals(checkDto.getTaiyoHituyo())) {
				if (NfwStringUtils.isBlank(checkDto.getHitsuyoRiyu())) {
					ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1054, "社宅を必要とする理由");
					checkDto.setHitsuyoRiyuErr(validationErrorCode);
					result = false;
				}
			}
		}
		...
		if (NfwStringUtils.isNotBlank(checkDto.getTaiyoHituyo())) {
			if (CodeConstant.ASKED_SHATAKU_FUYOU.equals(checkDto.getTaiyoHituyo())) {
				if (NfwStringUtils.isBlank(checkDto.getFuhitsuyoRiyu())) {
					ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1054, "社宅を必要としない理由");
					checkDto.setFuhitsuyoRiyuErr(validationErrorCode);
					result = false;
				}
			}
		}
		...
		if (NfwStringUtils.isNotBlank(checkDto.getHitsuyoRiyu())
			...
			if (NfwStringUtils.isBlank(checkDto.getAgencyCd())) {
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1054, "機関");
				checkDto.setNewAgencyErr(validationErrorCode);
				result = false;
			}
			...
			if (CodeConstant.RELATION_OTHERS.equals(checkDto.getAffiliation1Cd())
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "部 その他");
				checkDto.setOtherAffiliation1Err(validationErrorCode);
				result = false;
			}
			...
			if (CodeConstant.RELATION_OTHERS.equals(checkDto.getAffiliation2Cd())
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "室、チーム又は課 その他");
				checkDto.setOtherAffiliation2Err(validationErrorCode);
				result = false;
			}
		}
		...
		if (NfwStringUtils.isNotBlank(checkDto.getTaiyoHituyo())
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1054, "必要とする社宅 (世帯、単身、独身)");
			checkDto.setHitsuyoShatakuErr(validationErrorCode);
			result = false;
		}
		...
		if (CodeConstant.ASKED_SHATAKU_HITSUYO.equals(checkDto.getTaiyoHituyo())
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "入居希望日(予定日)");
			checkDto.setNyukyoYoteiDateErr(validationErrorCode);
			result = false;
		...
		if (NfwStringUtils.isNotBlank(checkDto.getTaiyoHituyo())
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1054, "自動車の保管場所");
			checkDto.setParkingUmuErr(validationErrorCode);
			result = false;
		}
			...
			if (bCheckNeedCar1 == true) {
				...
				if (NfwStringUtils.isBlank(checkDto.getCarNoInputFlg())
					...
					if (NfwStringUtils.isBlank(checkDto.getCarName())) {
						ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
								"自動車の車名((１台目)");
						checkDto.setCarNameErr(validationErrorCode);
						result = false;
					}
					// 自動車の登録番号(１台目)
					...
					if (NfwStringUtils.isBlank(checkDto.getCarNo())) {
						ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
								"自動車の登録番号(１台目)");
						checkDto.setCarNoErr(validationErrorCode);
						result = false;
					}
					// 車検の有効期間満了日(１台目)
					...
					if (NfwStringUtils.isBlank(checkDto.getCarExpirationDate())) {
						ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
								"車検の有効期間満了日(１台目)");
						checkDto.setCarExpirationDateErr(validationErrorCode);
						result = false;
					}
					// 自動車の使用者(１台目)
					...
					if (NfwStringUtils.isBlank(checkDto.getCarUser())) {
						ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
								"自動車の使用者(１台目)");
						checkDto.setCarUserErr(validationErrorCode);
						result = false;
					}
					// 自動車の保管場所使用開始日(１台目)
					...
					if (NfwStringUtils.isBlank(checkDto.getParkingUseDate())) {
						ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
								"車検の有効期間満了日(１台目)");
						checkDto.setParkingUseDateErr(validationErrorCode);
						result = false;
					}
					...
					if (NfwStringUtils.isBlank(checkDto.getCarNoInputFlg())) {
						ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1054,
								"自動車の保有(１台目)");
						checkDto.setCarNoInputFlgErr(validationErrorCode);
						result = false;
					}
					// 自動車の車名
				...
				if (NfwStringUtils.isNotBlank(checkDto.getCarName2()) || NfwStringUtils.isNotBlank(checkDto.getCarNo2())
					...
					if (NfwStringUtils.isNotBlank(checkDto.getCarNoInputFlg2())
						...
						if (NfwStringUtils.isBlank(checkDto.getCarName2())) {
							ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
									"自動車の車名(2台目)");
							checkDto.setCarName2Err(validationErrorCode);
							result = false;
						}
						// 自動車の登録番号(2台目)
						...
						if (NfwStringUtils.isBlank(checkDto.getCarNo2())) {
							ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
									"自動車の登録番号(2台目)");
							checkDto.setCarNo2Err(validationErrorCode);
							result = false;
						}
						// 車検の有効期間満了日(2台目)
						...
						if (NfwStringUtils.isBlank(checkDto.getCarExpirationDate2())) {
							ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
									"車検の有効期間満了日(2台目)");
							checkDto.setCarExpirationDate2Err(validationErrorCode);
							result = false;
						}
						// 自動車の使用者(2台目)
						...
						if (NfwStringUtils.isBlank(checkDto.getCarUser2())) {
							ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
									"自動車の使用者(2台目)");
							checkDto.setCarUser2Err(validationErrorCode);
							result = false;
						}
						// 自動車の保管場所使用開始日(2台目)
						...
						if (NfwStringUtils.isBlank(checkDto.getParkingUseDate2())) {
							ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
									"車検の有効期間満了日(2台目)");
							checkDto.setParkingUseDate2Err(validationErrorCode);
							result = false;
						}
						...
						if (NfwStringUtils.isBlank(checkDto.getCarNoInputFlg2())) {
							ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1054,
									"自動車の保有(2台目)");
							checkDto.setCarNoInputFlg2Err(validationErrorCode);
							result = false;
						}
						// 自動車の車名(2台目)
						LogUtils.debugByMsg(msg + "自動車の車名(2台目)" + checkDto.getCarName());
		...
		if (checkDto.getShatakuKanriId() < 0) {
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1054, "現居住宅");
			checkDto.setNowShatakuErr(validationErrorCode);
			result = false;
		}
		...
		if ((checkDto.getShatakuKanriId() > 0) && CodeConstant.HOYU.equals(checkDto.getNowShataku())) {
			...
			if (checkDto.getShatakuKanriId() == 0) {
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "保有社宅名");
				checkDto.setNowShatakuNameErr(validationErrorCode);
				result = false;
			}

			// 現保有の社宅
			LogUtils.debugByMsg(msg + "現保有の社宅" + checkDto.getTaiyoHituyo() + checkDto.getTaikyoYotei());
			if ((!(CodeConstant.ASKED_SHATAKU_PARKING_ONLY.equals(checkDto.getTaiyoHituyo())))
					&& NfwStringUtils.isBlank(checkDto.getTaikyoYotei())) {
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1054, "現保有の社宅");
				checkDto.setTaikyoYoteiErr(validationErrorCode);
				result = false;
			}
			...
			if (CodeConstant.LEAVE.equals(checkDto.getTaikyoYotei())) {
				...
				if (NfwStringUtils.isBlank(checkDto.getTaikyoYoteiDate())) {
					ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "退居予定日");
					checkDto.setTaikyoYoteiDateErr(validationErrorCode);
					result = false;
				}
				...
				if (NfwStringUtils.isNotBlank(checkDto.getTaiyoHituyo())
					...
					if (NfwStringUtils.isBlank(checkDto.getTaikyoRiyuKbn())) {
						ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "退居理由");
						checkDto.setDdlTaikyoRiyuKbnListErr(validationErrorCode);
						result = false;
					}
					// 退居理由がその他の場合
					...
					if (NfwStringUtils.isNotBlank(checkDto.getTaikyoRiyuKbn())) {
						if (CodeConstant.OTHER_RIYU_VALUE.equals(checkDto.getTaikyoRiyuKbn())) {
							if (NfwStringUtils.isBlank(checkDto.getTaikyoRiyu())) {
								ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
										"退居理由");
								checkDto.setTaikyoRiyuErr(validationErrorCode);
								result = false;
							}
						}
					}
					...
					if (NfwStringUtils.isBlank(checkDto.getTaikyogoRenrakuSaki())) {
						ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "退居後連絡先");
						checkDto.setTaikyogoRenrakuSakiErr(validationErrorCode);
						result = false;
					}
					// 備品返却がある場合のみチェック
					...
					if (NfwStringUtils.isNotBlank(checkDto.getHdnBihinHenkyakuUmu())
						...
						if (NfwStringUtils.isBlank(checkDto.getSessionDay())) {
							ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
									"返却立会希望日(日)");
							checkDto.setSessionDayErr(validationErrorCode);
							result = false;
						}
						// 返却立会希望日(時)
						...
						if (NfwStringUtils.isBlank(checkDto.getSessionTime())) {
							ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048,
									"返却立会希望日(時)");
							checkDto.setSessionTimeErr(validationErrorCode);
							result = false;
						}
						// 連絡先
						LogUtils.debugByMsg(msg + "連絡先" + checkDto.getRenrakuSaki());
						if (NfwStringUtils.isBlank(checkDto.getRenrakuSaki())) {
							ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "連絡先");
							checkDto.setRenrakuSakiErr(validationErrorCode);
							result = false;
						}
					}
				}
	...
	private boolean checkEmptyKazokuInfo(Skf2020Sc002CheckAsyncDto checkDto) {
		...
		if (!((NfwStringUtils.isEmpty(checkDto.getDokyoRelation1()) && NfwStringUtils.isEmpty(checkDto.getDokyoName1())
			...
			if (NfwStringUtils.isBlank(checkDto.getDokyoRelation1())) {
				checkDto.setDokyoRelation1Err(validationErrorCode);
				isZokugaraError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoName1())) {
				checkDto.setDokyoName1Err(validationErrorCode);
				isShimeiError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoAge1())) {
				checkDto.setDokyoAge1Err(validationErrorCode);
				isNenreiError = true;
			}
		}
		...
		if (!((NfwStringUtils.isEmpty(checkDto.getDokyoRelation2()) && NfwStringUtils.isEmpty(checkDto.getDokyoName2())
			...
			if (NfwStringUtils.isBlank(checkDto.getDokyoRelation2())) {
				checkDto.setDokyoRelation2Err(validationErrorCode);
				isZokugaraError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoName2())) {
				checkDto.setDokyoName2Err(validationErrorCode);
				isShimeiError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoAge2())) {
				checkDto.setDokyoAge2Err(validationErrorCode);
				isNenreiError = true;
			}
		}
		...
		if (!((NfwStringUtils.isEmpty(checkDto.getDokyoRelation3()) && NfwStringUtils.isEmpty(checkDto.getDokyoName3())
			...
			if (NfwStringUtils.isBlank(checkDto.getDokyoRelation3())) {
				checkDto.setDokyoRelation3Err(validationErrorCode);
				isZokugaraError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoName3())) {
				checkDto.setDokyoName3Err(validationErrorCode);
				isShimeiError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoAge3())) {
				checkDto.setDokyoAge3Err(validationErrorCode);
				isNenreiError = true;
			}
		}
		...
		if (!((NfwStringUtils.isEmpty(checkDto.getDokyoRelation4()) && NfwStringUtils.isEmpty(checkDto.getDokyoName4())
			...
			if (NfwStringUtils.isBlank(checkDto.getDokyoRelation4())) {
				checkDto.setDokyoRelation4Err(validationErrorCode);
				isZokugaraError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoName4())) {
				checkDto.setDokyoName4Err(validationErrorCode);
				isShimeiError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoAge4())) {
				checkDto.setDokyoAge4Err(validationErrorCode);
				isNenreiError = true;
			}
		}
		...
		if (!((NfwStringUtils.isEmpty(checkDto.getDokyoRelation5()) && NfwStringUtils.isEmpty(checkDto.getDokyoName5())
			...
			if (NfwStringUtils.isBlank(checkDto.getDokyoRelation5())) {
				checkDto.setDokyoRelation5Err(validationErrorCode);
				isZokugaraError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoName5())) {
				checkDto.setDokyoName5Err(validationErrorCode);
				isShimeiError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoAge5())) {
				checkDto.setDokyoAge5Err(validationErrorCode);
				isNenreiError = true;
			}
		}
		...
		if (!((NfwStringUtils.isEmpty(checkDto.getDokyoRelation6()) && NfwStringUtils.isEmpty(checkDto.getDokyoName6())
			...
			if (NfwStringUtils.isBlank(checkDto.getDokyoRelation6())) {
				checkDto.setDokyoRelation6Err(validationErrorCode);
				isZokugaraError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoName6())) {
				checkDto.setDokyoName6Err(validationErrorCode);
				isShimeiError = true;
			}
			if (NfwStringUtils.isBlank(checkDto.getDokyoAge6())) {
				checkDto.setDokyoAge6Err(validationErrorCode);
				isNenreiError = true;
			}
		}
		...
		// 続柄エラー判定
		if (isZokugaraError) {
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "続柄");
		}
		// 氏名エラー判定
		if (isShimeiError) {
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "氏名");
		}
		// 年齢エラー判定
		if (isNenreiError) {
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1048, "年齢");
		}
	...
	private Boolean checkValueType(Skf2020Sc002CheckAsyncDto checkDto) {
		...
		if ((NfwStringUtils.isNotBlank(checkDto.getTel()))
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1003, "勤務先のTEL");
			checkDto.setTelErr(validationErrorCode);
			result = false;
		}
		...
		if ((NfwStringUtils.isNotBlank(checkDto.getHitsuyoShataku()))
			...
			if (isError) {
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1050, "年齢");
				result = false;
			}
		}
		...
		if (NfwStringUtils.isNotBlank(checkDto.getRenrakuSaki())
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1003, "連絡先");
			checkDto.setRenrakuSakiErr(validationErrorCode);
		}
	...
	private Boolean checkByteCount(Skf2020Sc002CheckAsyncDto checkDto) throws UnsupportedEncodingException {
		...
		if (NfwStringUtils.isNotBlank(checkDto.getNewAffiliation1Other())
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "勤務先のTEL", "13");
			checkDto.setTelErr(validationErrorCode);
			result = false;
		}
		...
		if ((NfwStringUtils.isNotBlank(checkDto.getAffiliation1Cd())
			...
			if (NfwStringUtils.isNotBlank(checkDto.getNewAffiliation1Other())
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "新所属 部等-その他", "64");
				checkDto.setOtherAffiliation1Err(validationErrorCode);
				result = false;
			}
		}
		...
		if ((NfwStringUtils.isNotBlank(checkDto.getAffiliation2Cd())
			...
			if (NfwStringUtils.isNotBlank(checkDto.getNewAffiliation2Other())
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "新所属 室チームまたは課-その他",
						"64");
				checkDto.setOtherAffiliation2Err(validationErrorCode);
				result = false;
			}
		...
		if (CodeConstant.CAR_PARK_HITUYO.equals(checkDto.getParkingUmu())) {
			...
			if (NfwStringUtils.isNotBlank(checkDto.getCarName())
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "自動車の車名", "33");
				checkDto.setCarNameErr(validationErrorCode);
				result = false;
			}
			// 自動車の登録番号
			...
			if (NfwStringUtils.isNotBlank(checkDto.getCarNo())
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "自動車の登録番号", "33");
				checkDto.setCarNoErr(validationErrorCode);
				result = false;
			}
			// 自動車の使用者
			...
			if (NfwStringUtils.isNotBlank(checkDto.getCarUser())
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "自動車の使用者", "33");
				checkDto.setCarUserErr(validationErrorCode);
				result = false;
			}
		}
		...
		if (CodeConstant.LEAVE.equals(checkDto.getTaikyoYotei())) {
			...
			if (NfwStringUtils.isNotBlank(checkDto.getShatakuJyotai())
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "社宅の状態", "128");
				checkDto.setShatakuJyotaiErr(validationErrorCode);
				result = false;
			}
			// 退居理由がその他の場合
			...
			if (NfwStringUtils.isNotBlank(checkDto.getTaikyoRiyuKbn())
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "退居理由", "126");
				checkDto.setTaikyoRiyuErr(validationErrorCode);
				result = false;
			}
			// 退居後の連絡先
			...
			if (NfwStringUtils.isNotBlank(checkDto.getTaikyogoRenrakuSaki())
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "退居後の連絡先", "64");
				checkDto.setTaikyogoRenrakuSakiErr(validationErrorCode);
				result = false;
			}
			// 特殊事情など
			...
			if (NfwStringUtils.isNotBlank(checkDto.getTokushuJijo())
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "特殊事情など", "128");
				checkDto.setTokushuJijoErr(validationErrorCode);
				result = false;
			}
		}
		...
		if (NfwStringUtils.isNotBlank(checkDto.getRenrakuSaki())
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "連絡先", "13");
			checkDto.setRenrakuSakiErr(validationErrorCode);
			result = false;
		}
		return result;
	...
	private boolean checkByteCountZokugara(Skf2020Sc002CheckAsyncDto checkDto) throws UnsupportedEncodingException {
		...
		// エラーメッセージの追加
		if (isError) {
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "同居家族　続柄", "5");
			result = false;
		}
		return result;
	}
	...
	private boolean checkByteCountShimei(Skf2020Sc002CheckAsyncDto checkDto) throws UnsupportedEncodingException {
		...
		// エラーメッセージの追加
		if (isError) {
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1071, "同居家族　氏名", "13");
			result = false;
		}
		return result;
	}
	...
	private boolean checkByteCountNenrei(Skf2020Sc002CheckAsyncDto checkDto) throws UnsupportedEncodingException {
		...
		// エラーメッセージの追加
		if (isError) {
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1049, "同居家族　年齢", "3");
			result = false;
		}
		return result;
	}
	...
	private Boolean checkDateFormat(Skf2020Sc002CheckAsyncDto checkDto) {
		...
		boolean result = true;
		// システム日付を文字列に変換
		String sysDate = DateUtils.getSysDateString("yyyy/MM/dd");
		...
		if ((NfwStringUtils.isNotBlank(checkDto.getTaiyoHituyo())
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1055, "入居希望日(予定日)");
			checkDto.setNyukyoYoteiDateErr(validationErrorCode);
			result = false;
		}
		...
		if ((NfwStringUtils.isNotBlank(checkDto.getCarExpirationDate())) && !(SkfCheckUtils
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1055, "車検の有効期間満了日(1台目)");
			checkDto.setCarExpirationDate2Err(validationErrorCode);
			result = false;
		}
		// 当日未満ならエラー
		if ((NfwStringUtils.isNotBlank(checkDto.getCarExpirationDate()))
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_2017, "車検の有効期間満了日(1台目)");
			checkDto.setCarExpirationDateErr(validationErrorCode);
			result = false;
		}
		// 車検の有効期間満了日(２台目)
		...
		if ((NfwStringUtils.isNotBlank(checkDto.getCarExpirationDate2())) && !(SkfCheckUtils
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1055, "車検の有効期間満了日(2台目)");
			checkDto.setCarExpirationDate2Err(validationErrorCode);
			result = false;
		}
		// 当日未満ならエラー
		if ((NfwStringUtils.isNotBlank(checkDto.getCarExpirationDate2()))
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_2017, "車検の有効期間満了日(2台目)");
			checkDto.setCarExpirationDate2Err(validationErrorCode);
			result = false;
		}
		...
		if ((NfwStringUtils.isNotBlank(checkDto.getCarExpirationDate())) && !(SkfCheckUtils
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1055, "保管場所使用開始日(1台目)");
			checkDto.setParkingUseDateErr(validationErrorCode);
			result = false;
		}
		// 保管場所使用開始日(2台目)
		...
		if ((NfwStringUtils.isNotBlank(checkDto.getCarExpirationDate2())) && !(SkfCheckUtils
			...
			ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1055, "保管場所使用開始日(2台目)");
			checkDto.setParkingUseDate2Err(validationErrorCode);
			result = false;
		}
		...
		if (CodeConstant.LEAVE.equals(checkDto.getTaikyoYotei())) {
			...
			if ((NfwStringUtils.isNotBlank(checkDto.getTaikyoYoteiDate())) && !(SkfCheckUtils
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1055, "退居予定日");
				checkDto.setTaikyoYoteiDateErr(validationErrorCode);
				result = false;
			}
			// 返却立会希望日(日)
			...
			if ((NfwStringUtils.isNotBlank(checkDto.getSessionDay())) && !(SkfCheckUtils
				...
				ServiceHelper.addErrorResultMessage(checkDto, null, MessageIdConstant.E_SKF_1055, "返却立会希望日(日)");
				checkDto.setSessionDayErr(validationErrorCode);
				result = false;
			}
		}
