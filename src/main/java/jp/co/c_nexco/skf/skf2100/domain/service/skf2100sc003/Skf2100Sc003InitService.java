...
import jp.co.c_nexco.skf.skf2100.domain.dto.skf2100sc003.Skf2100Sc003InitDto;
...
/**
 * Skf2100Sc001 モバイルルーター返却申請書（申請者用)初期表示処理クラス
 *
 * @author NEXCOシステムズ
 */
@Service
public class Skf2100Sc003InitService extends SkfServiceAbstract<Skf2100Sc003InitDto> {
	 ...
	 */
	@Override
	public Skf2100Sc003InitDto index(Skf2100Sc003InitDto initDto) throws Exception {
		// 操作ログ出力
		skfOperationLogUtils.setAccessLog("初期表示", CodeConstant.C001, FunctionIdConstant.SKF2100_SC003);
		// セッション情報引き渡し
		skf2100Sc003SharedService.setMenuScopeSessionBean(menuScopeSessionBean);
		// セッション情報初期化
		skf2100Sc003SharedService.clearMenuScopeSessionBean();

		// タイトル設定
		initDto.setPageTitleKey(MessageIdConstant.SKF2100_SC003_TITLE);

		skf2100Sc003SharedService.setClearInfo(initDto);
		...
		// 画面内容の設定
		skf2100Sc003SharedService.initializeDisp(initDto);


		// 画面制御処理（活性／非活性）
		skf2100Sc003SharedService.setControlValue(initDto);
		
		// 操作ガイドの設定
		initDto.setOperationGuide(skfOperationGuideUtils.getOperationGuide(FunctionIdConstant.SKF2100_SC003));

		
		// コメント設定の有無
		if (skf2100Sc003SharedService.setCommentBtnDisabled(initDto.getApplNo())) {
			// コメントがあれば表示
	...
	private void setUserInfo(Skf2100Sc003InitDto initDto) {
		// ユーザコンテキストを取得
