 ...
 */
package jp.co.c_nexco.skf.skf2050.domain.service.skf2050fc001;
...
import jp.co.c_nexco.skf.skf2050.domain.dto.skf2050fc001.Skf2050Fc001ExecuteDto;
 ...
 */
@Service
public class Skf2050Fc001ExecuteService extends BaseServiceAbstract<Skf2050Fc001ExecuteDto> {

	// マップのキーはSessionCacheKeyConstantクラスに定義すること！
	private static final String DATA_LINKAGE_KEY_SKF2050FC001 = "keyForUpdate";

	@Autowired
	SkfBatchUtils skfBatchUtils;
	@Autowired
	Skf2050Fc001BihinHenkyakuSinseiDataImport skf2050Fc001;

	/**
	 * 画面初期表示のメイン処理
	 */
	@Override
	public BaseDto index(Skf2050Fc001ExecuteDto dto) throws Exception {

		LogUtils.debugByMsg("Skf2050Fc001-UTTest：テスト実行クラス処理開始");

		// 「null」と入力されている場合NULL値に置き換える
		String companyCd = dto.getSkf2050Fc001CompanyCd();
		String shainNo = dto.getSkf2050Fc001ShainNo();
		String applNo = dto.getSkf2050Fc001ApplNo();
		String applStatus = dto.getSkf2050Fc001ApplStatus();

		long startTime = System.currentTimeMillis();

		/** 退居のデータ連携機能を例とする */
		// menuScopeSessionBeanからオブジェクトを取得する
		Object forUpdateObject = menuScopeSessionBean.get(DATA_LINKAGE_KEY_SKF2050FC001);

		// 取得したオブジェクトを専用メソッドを使ってダウンキャストする
		Map<String, List<SkfBatchUtilsGetMultipleTablesUpdateDateExp>> forUpdateMap = skf2050Fc001
				.forUpdateMapDownCaster(forUpdateObject);

		// ダウンキャストしたMapをデータ連携クラスに格納する
		skf2050Fc001.setUpdateDateForUpdateSQL(forUpdateMap);

		// データ連携機能をキックする
		List<String> resultList = skf2050Fc001.doProc(companyCd, shainNo, applNo, applStatus,
				dto.getSkf2050Fc001UserID(), dto.getSkf2050Fc001PageID());

		// セッションに保持しているデータ連携用のMapを削除する
		menuScopeSessionBean.remove(DATA_LINKAGE_KEY_SKF2050FC001);

		if (resultList != null) {
			// エラーメッセージ出力
			skf2050Fc001.addResultMessageForDataLinkage(dto, resultList);
		...
		LogUtils.debugByMsg("Skf2050Fc001-UTTest：処理にかかった時間：" + (endTime - startTime) + "ミリ秒");
		LogUtils.debugByMsg("Skf2050Fc001-UTTest：テスト実行クラス処理終了");
		return dto;
	}
